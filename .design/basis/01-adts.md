...
| REQ-5 (exhaustiveness checking in the validator) | NOT-STARTED | epic **#62** Stage 1b. `validator.rs` has no enum-variant-set collection and no `NonExhaustiveMatch`/`UnreachableArm` `SpecError` variant. Stage 1a landed the honest GATE (`SpecError::UnsupportedAdt`, returned from `validate`'s `Item::Struct`/`Item::Enum`/`Expr::StructLit`/`Expr::Is`/`Expr::Deref` arms) — every ADT program PARSES then dies at validation with a structured scream; 1b REPLACES that gate with the real exhaustiveness logic this REQ specifies. |
| REQ-6 (well-formed field/variant access + `is`) | NOT-STARTED | epic **#62** Stage 1. No `Expr::Is` in `ast.rs`; `validator.rs` has no field/variant well-formedness check. `s is Circle` GROUNDED in Verus. |
| REQ-7 (ADT predicates fit the SpecTherm cage) | NOT-STARTED | epic **#62** Stage 1. The caged-flat walk (`.design/spec/spectherm-combinators.md` REQ-6) is itself NOT-STARTED (blocker #40); ADT built-ins (`Field`/`Match`/`If`/`Is`) join its flat accept set when both land. |
| REQ-8 (struct → Verus struct; invariant → predicate) | NOT-STARTED | epic **#62** Stage 1. `lower.rs` has no `lower_struct`/`well_formed`-predicate emission. GROUNDED verified form pinned (Architecture); visibility-tier finding recorded. |
