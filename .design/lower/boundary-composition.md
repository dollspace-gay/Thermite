...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (assumable-signature emission, boundary/slag only) | NOT-STARTED | open prereq blocker #52 (found during #17). No `external_body` emission exists in `thermite-lower/src/lower.rs`: `grep external_body` over `lower.rs` is empty; `lower_fn_body in lower.rs` returns `LowerError::Unsupported` on a `body: None` boundary fn ("lower (L3 Verus) reached a bodyless (boundary) fn") rather than emitting an assumable signature. The honesty gate (external_body iff `#[boundary]`/`#[slag]`) is unimplemented because no emission exists. Mechanism grounded: `#[verifier::external_body] fn ext_id(..) requires x<100, ensures r==x` lets a caller prove L3 (`verus 0.2026.05.24`: `success: true, verified: 1, errors: 0`). |
| REQ-2 (boundary-caller reaches L3 + scope `to_boundary`) | NOT-STARTED | open prereq blocker #52. `item_subprogram in forge/src/check.rs` weaves ONLY `spec_items` (the `Item::SpecFn` filter in `check_file_with_options`) into a caller `fn`'s sub-program, never the referenced `#[boundary]`/`#[slag]` siblings, so `lower` emits an UNDEFINED callee and `verus` errors → the `direct_boundary_caller` `caller` lands `L0` today (the `conformance/e2e/cases.json` fixture). #17's `closure::classify` already attaches `assurance_scope = ToBoundary { via }` (SHIPPED) — the LEVEL is the open gap. |
| REQ-3 (boundary/slag fn itself unchanged — L1 + flag) | NOT-STARTED | open prereq blocker #52. The §16/§8 path is SHIPPED and #52 must NOT touch it: `gate_fn in check.rs` short-circuits a `f.boundary.is_some()` item to `GateOutcome::BoundaryL1` (`Certificate::boundary_l1`, `Level::L1`, `boundary: true`, no verus) and a `f.slag.is_some()` item to `SlagL1`, both BEFORE the L3 path. This REQ pins that #52's composition weaving leaves `f`'s own cert byte-stable; the invariant is unverified until #52's oracle asserts `ext_id`'s L1 cert unchanged. |
| REQ-4 (soundness — violation is a counterexample, not a false L3) | NOT-STARTED | open prereq blocker #52. No composition path exists, so the soundness invariant is unverified in-tree. Mechanism grounded against `verus 0.2026.05.24`: a caller not establishing `ext_id`'s `req` → `precondition not satisfied` (0 verified, 1 error); a caller asserting `ens result == x + 1` (stronger than the contract) → `postcondition not satisfied` (0 verified, 1 error) — both COUNTEREXAMPLES, never a false L3. The external_body BODY itself is unchecked (a lying `{ x + 1 }` body still verifies `0/0`), but a REGULAR fn with that body FAILS — the honesty boundary (REQ-1) is what keeps this sound. |
