...
- **REQ-4 (step-2.2.2 loops — the harder horizon, FRAMED not designed)** — the after-loop state needs
  the loop invariant / bounded unrolling / a fixpoint; the candidate approaches + why kernel-gated are
  framed in "Step 2.2.2 horizon" below. NOT designed here. **Blocker #163.**
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (frozen kernel exec-statement subset v1) | NOT-STARTED | open epic/prereq blocker #158. The IN/OUT construct set is ENUMERATED above from the existing `enum Stmt`/`struct Block`/`struct LoopNode in thermite-syntax/src/ast.rs` (lowered by `lower_stmt`/`lower_block_inner`/`lower_loop in thermite-lower/src/lower.rs`), but it is not yet PINNED in code as a stable, design-amendment-gated set — no `thermite-tv` artifact references a frozen subset constant. |
| REQ-2 (operational-semantics reference state-denotation) | NOT-STARTED | open prereq blocker #159. No `thermite_tv::exec_stmt_encode::body_ref_state` exists (`thermite-tv/src/` has `exec_encode.rs` for EXPRESSIONS only; `grep` for `body_ref`/`exec_stmt`/`state_denotation` in `thermite-tv/src/` is empty). The design + the independence boundary (composes `exec_ref_value`, no `thermite-lower` dep) are pinned above; the encoder is unbuilt. |
| REQ-3 (step-2.2.1 straight-line body state-refinement obligation + discharge) | NOT-STARTED | open prereq blockers #160 (`thermite_tv::obligation::body_equivalence_obligation` — unbuilt; `obligation.rs` has `exec_equivalence_obligation` for a single VALUE only) + #161 (`thermite_lower::lower_exec_body` per-body exec entry — unbuilt; `lower_block_inner in lower.rs` is private/fn-context-bound). The OBLIGATION SHAPE is GROUNDED end-to-end against real verus (Verification AC-1/2/3/4: faithful straight-line bodies VERIFY; dropped-statement / reordered-mutation / swapped-branch infidelities → `postcondition not satisfied`) — feasibility proven, code unbuilt. |
| REQ-4 (step-2.2.2 loops — harder horizon) | NOT-STARTED | open prereq blocker #163. Kernel-gated (REQ-1 frozen-subset prerequisite). Candidate approaches (single-iteration step-TV + production invariant; bounded unrolling) FRAMED above; deliberately NOT designed (the moving-target argument). |
| REQ-5 (forge `body_tv` plug-in point) | NOT-STARTED | open prereq blocker #162. No `forge::body_tv` module (`forge/src/` has `exec_tv.rs` for EXPRESSIONS; `grep` for `body_tv`/`run_body_tv` in `forge/src/` is empty). The four-way `Faithful`/`Divergent`/`Unverifiable`/`Skipped` plug-in (sibling to `forge::exec_tv::run_generated`) is designed above; unbuilt. Depends on REQ-2/REQ-3. |
