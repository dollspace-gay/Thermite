...
- **AC-7 (loops / out-of-scope bodies skipped HONESTLY)** — a body containing an out-of-v1 `Stmt::Loop`,
  an aggregate mutation outside the fixed mutable-slice subset, or a mid-body early return reaches the `forge::body_tv` phase as `Skipped` (with
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (frozen kernel exec-statement subset v1) | SHIPPED | the IN/OUT construct set is PINNED IN CODE: `thermite_tv::exec_stmt_encode::body_ref_state` (and its `thread_stmt`/`encode_value`) ADMIT `Stmt::Let`/bare-scalar `Assign`/top-level single-index assignment to a frame-declared mutable slice/`If`/`Expr`/tail-`Return` + `Block` sequencing/tail. The slice-effect subset requires indices/values not to read a mutable slice and rejects nested slice writes, keeping pre/final state meanings distinct. It HONESTLY REJECTS (an `Unsupported` `Err`) `Stmt::Loop`/`Break`/`Continue` (2.2.2), a mid-`if`-branch early return, `match`-stmt, other aggregate mutation, and a re-shadow. Verified by B1–B4, honest-skip tests, the real `service_write_user_byte` body-TV pass, and mutable-slice wrong-index/wrong-value teeth. |
| REQ-2 (operational-semantics reference state-denotation) | SHIPPED | `pub fn body_ref_state` + `body_ref_state_ensures`/`slice_write_ensures`/`BodyRefCtx` in `thermite-tv/src/exec_stmt_encode.rs` — the big-step state-transformer (let/assign substitution-threading, mutation-ORDER sensitivity, `if`-branch composition, multi-cell tuple projection) plus a complete mutable-slice sequence transition from `old(slice)@` through an ordered `.update(...)` chain to `final(slice)@`, composing step-2.1's `exec_ref_value` on each env-substituted value. Non-test consumer: `thermite_tv::obligation::body_equivalence_obligation`. Independence is STRUCTURAL: deps `thermite-syntax` + `thermite-spec` ONLY (`cargo tree -p thermite-tv` — no `thermite-lower`, AC-6). The corresponding Lean state model is `Stmt.sliceAssign`/`State.setSlice` with `refStmt_eq_stmtDenote`; wrong-index and wrong-value theorems bite. |
