/-
  ...
  that is not the §4.2.2 sketch. The builder declared one adaptation
  ...
  1. `shipped_hdec_is_not_the_pinned_shape` — a concrete instance (the shipped L1
     ...
     and the shipped theorem is a different statement from the pinned one.
...
/-- The shipped `h_dec` hypothesis shape (`Exec/WhileBody.lean`,
    `loopDenote_exits_of_dec`): descent plus the pre-state bound `0 ≤ μ st`. -/
...
/-- The design-pinned `h_dec` hypothesis shape (proof-backends.md §4.2.2, quoted
    verbatim): descent plus the post-state bound `0 ≤ μ st'`. -/
...
/-- The pin — the two `h_dec` shapes are not equivalent (the "verbatim" claim is
    ...
    the design-pinned one, so `loopDenote_exits_of_dec` as shipped is a
...
/-- The design's verbatim theorem (§4.2.2's `0 ≤ μ st'` shape) is a one-line
    ...
    "verbatim" claim is false), not a soundness hole, and the (v-b) exporter can target
