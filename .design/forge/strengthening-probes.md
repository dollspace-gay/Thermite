...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (frozen deterministic bounded candidate template) | NOT-STARTED | open prereq blocker #14. `strengthen.rs` does not exist; no `generate_candidates` / `CANDIDATE_CAP` / candidate-clause `Expr` builder is implemented. The inputs (`thermite_syntax::{Expr, BinOp, Clause}`, `mutation::MutationScore`) all ship. |
| REQ-2 (verify candidate vs real body, reuse `run_verus`) | NOT-STARTED | open prereq blocker #14. No `verify_candidate`; `check::run_verus` + `item_subprogram` ship (`private` in `check.rs`) and are the seam to reuse, but nothing calls them for a candidate `ens`. |
| REQ-3 (strictly-stronger filter) | NOT-STARTED | open prereq blocker #14. No `is_strictly_stronger`; the survivor-kill link (#12 `MutationScore.survivor`) and the clause AST exist as inputs, unused. |
| REQ-4 (advisory placement — additive cert field, not a gate) | NOT-STARTED | open prereq blocker #14. No `manifest::Suggestion` / `strengthening` field; the `suggested_move` slot (`manifest::SuggestedMove`) ships reserved but is populated only on a timeout cert (#11), never by a probe. |
| REQ-5 (consumes #12 survivors; runs on a settled L3 + scored item) | NOT-STARTED | open prereq blocker #14. `check::check_file_with_options` runs `mutation_score` after L3 but has NO post-scoring probe call; the survivors are computed and dropped, not fed to a strengthener. |
| REQ-6 (determinism — same fn ⇒ same suggestions) | NOT-STARTED | open prereq blocker #14. No `generate_candidates` to be deterministic; the pinned seed + #8 cache that would make each candidate verdict stable ship and are the mechanism, but unused by any probe. |
