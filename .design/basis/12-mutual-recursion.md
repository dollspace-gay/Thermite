...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (mutual cycle with `dec` on every member → L3) | NOT-STARTED | open prereq blocker #121 (epic #113). `forge::check`'s `mutual_recursion_cycle_fns` (in `check.rs`) UNCONDITIONALLY rejects EVERY size-≥2 cycle member as `Level::L0`/`MutualRecursionUnsupported` (the #110 blanket guard) regardless of `dec` — so a dec-complete `is_even`/`is_odd` pair is rejected at `forge check` before lowering, never reaching the L3 ladder. GROUNDED that the dec-complete cycle WOULD certify L3 (Verus `4 verified, 0 errors`, Verification (A)), so the gap is purely the conditional in `mutual_recursion_cycle_fns`. |
| REQ-2 (the #110 reject becomes conditional — missing/insufficient `dec` → L0) | NOT-STARTED | open prereq blocker #121 (epic #113). `mutual_recursion_cycle_fns` rejects unconditionally with cause `MutualRecursionUnsupported` (`check.rs` per-item loop, `RejectReason { cause: "MutualRecursionUnsupported", .. }`); the refinement (reject ONLY when a cycle member `f.dec.is_none() && !fn_is_diverge(f)`, with a renamed `MutualRecursionMissingDecreases` cause + a member-naming detail) is not implemented. The `fx diverge` exemption (`fn_is_diverge` skip) already exists and is PRESERVED by the refinement. |
| REQ-3 (the mutual-group lowering — grandfathered-correct, no change) | SHIPPED | C9 #108. `lower` (in `lower.rs`) emits every `Item::Fn` in source order inside one `verus! { }` block; `lower_fn` emits `decreases <spec_dec(f.dec, f.params)>` for any `f.dec.is_some()`. GROUNDED that this is EXACTLY the form Verus accepts as a mutual group with NO adjacency requirement (Verification (D), `3 verified, 0 errors` with an interposed fn). Non-test consumer: `forge::check` (`item_subprogram` → `thermite_lower::lower` → `run_verus`). The lowering for the mutual group requires NO change — this REQ records the existing emission as the contract; the gap is solely `forge::check` gating the cycle off (REQ-1/REQ-2) BEFORE this lowering runs. |
| REQ-4 (v1 measure scope — full lexicographic, n-cycles, per-fn `dec`) | NOT-STARTED | open prereq blocker #121 (epic #113). GROUNDED that Verus supports the FULL capability — n-cycles (3-cycle → `3 verified`, Verification (A)/(E)) and lexicographic comma-list measures (`decreases n,1`/`decreases n,0` → `2 verified`) — but v1 is BLOCKED on REQ-1/REQ-2 (the `forge::check` conditional). The single-`dec`-per-member common case (`is_even`/`is_odd` on `n`) is the v1 target; the lexicographic comma-`dec` SURFACE is OQ-2 (Verus has the capability; the Thermite surface `dec` lowers a single measure today via `spec_dec`). |
