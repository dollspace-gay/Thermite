/-
  ...
  Int-position `specCall` to `0` (the `| none => 0` arm and the fuel-0 catch-all
  ...
  - `obligation_form_is_false`: the §4 `∀ fuel ≥ fuel₀ → req → ens` form,
    ...
    every finite fuel admits a falsifying env, and no fuel₀ computation fixes it.)
...
/-- The §4 obligation form (under-computed fuel₀ = 1, req = true) is false for
    this correct item; it is not padded with trivially-true conjuncts. -/
