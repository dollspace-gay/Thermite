...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (per-survivor Verus equivalence check) | NOT-STARTED | open blocker #101. `check::mutation_score` in `check.rs` classifies survivors (`mutant_outcome_is_survivor`) but issues NO follow-on equivalence query; the equivalence obligation is hand-derived + GROUNDED (above) but not yet a code seam. |
| REQ-2 (PROVED-equivalent → drop from denominator; sound-but-incomplete) | NOT-STARTED | open blocker #101. `mutation::MutationScore.scored` is incremented for every survived mutant (`mutation.rs` `MutationScore`; `check.rs` `scored += 1` unconditionally on a lowered+run mutant); no proved-equivalent drop exists. |
| REQ-3 (soundness line — distinguishing mutant never excluded) | NOT-STARTED | open blocker #101. The exclusion seam does not exist, so the soundness line (exclude only on a proof) is unenforced; GROUNDED that the `x + 1` and `loose` early-return survivors FAIL the equivalence query (above). |
| REQ-4 (`MutationScore` denominator + `K/N` cert) | NOT-STARTED | open blocker #101. `MutationScore::kill_ratio`/`mutants_killed_string` in `mutation.rs` read the un-reduced `scored`; no field records proved-equivalent exclusions. |
| REQ-5 (`CHECK_SCHEMA_VERSION` bump) | NOT-STARTED | open blocker #101. `cache::CHECK_SCHEMA_VERSION` is `4` (`cache.rs`); the verdict-changing exclusion requires a bump to invalidate stale forced-output verdicts. |
| REQ-6 (determinism + bounded per-survivor cost) | NOT-STARTED | open blocker #101. No equivalence-check path exists to be cached/deterministic yet; the formulation reuses the deterministic `run_verus` + `cache` seams (`check.rs` `mutation_score`). |
