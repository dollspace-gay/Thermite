...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (mandatory-field validation) | NOT-STARTED | open prereq blocker #6. `forge/src/slag.rs` does not exist; no `validate` over `SlagAttr`'s `Option<String>` fields for presence + non-emptiness (the parser leaves this "downstream/forge", `ast.rs` `SlagAttr`). |
| REQ-2 (L3-exempt / L1-enforced / `slag: true`) | NOT-STARTED | open prereq blocker #6. `check::check_file` always lowers + runs `verus`; `manifest.rs` hard-codes `slag: false` and never emits `Level::L1` for a slag item. |
| REQ-3 (slag justifies maximal `fx`) | NOT-STARTED | open prereq blocker #6 (and the §7.1 (d) half is blocked on `vacuity.rs`, same issue). No "slag present ⇒ rule (d) skipped" wiring exists. |
| REQ-4 (audit visibility — cert metadata) | NOT-STARTED | open prereq blocker #6. `Certificate` carries `slag: bool` only (no `slag_meta`); the metadata is never threaded into the cert (OQ-1 amendment pending). |
| REQ-5 (typed verdict + check integration) | NOT-STARTED | open prereq blocker #6. No `SlagVerdict`/`SlagError`; `check::check_file` has no slag gate stage and no L1 short-circuit fork. |
