...
//! | REQ-17 (`bytes_eq` — REGISTERED built-in spec predicate; surface + cage; NO skill entry) | SHIPPED | #278 cluster C8. `GENERATED_SPEC_FNS` += `bytes_eq` so a contract naming `bytes_eq(a, b, ai, bi, n)` (the `slice_id`/`insert_str` byte-content pins) validates inside the §4.2 cage as a named `spec fn` call — NOT a combinator-`REGISTRY` entry (its 5-arg String/index signature fits no `ArgKind`, no closure; and the generated-spec-fn path renders ZERO skill tokens, holding the §2.2 budget at 5988/6000 where a combinator entry would add ~+38 and blow the gate, REQ-17's verdict). The lowerer (`thermite-lower::lower::emit_bytes_eq_defs`) materializes the `Seq<u8>` low-peel def + the four prove-once bridge lemmas. Consumer: `pub fn validate` → `walk_call` (the `spec_fns` accept). Verified: `forge/tests/bytes_eq_conformance.rs` (the `slice_id` + 3-pin `insert_str` certify L3 under real verus; the head/tail-swap mutant FAILS, non-vacuous). |
//!
