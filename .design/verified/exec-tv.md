...
| REQ-3 (off-corpus generator — exec exprs) | NOT-STARTED | open prereq blocker #154 (NEXT dispatch — `gen_exec_exprs` not built; not on this manifest, independence-respecting). |
...
| REQ-5 (forge plug-in point) | NOT-STARTED | open prereq blocker #156 (NEXT dispatch — `forge/src/exec_tv.rs` not built; not on this manifest). It will run the exec-TV phase over body exec exprs via `thermite_lower::lower_exec_expr` (the EXEC-context per-expr entry now SHIPPED, this dispatch) and discharge the exec-fn obligation through `forge::check::run_verus`, mirroring `forge/src/contract_tv.rs`. The `lower_exec_expr` prerequisite is its eventual non-test production consumer (R-DEFER-1). |
