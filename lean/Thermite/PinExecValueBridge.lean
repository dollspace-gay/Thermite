/-
  ...
  faithful refutation. The file must keep compiling as the regression oracle.

  The witness is a straight-line body converging to `.int ⟨.u64, 2^64 − 1⟩` (the `u64`
  ...
  mathematical unsigned value, `Exec.lean`); `S_C` compares mathematical values.
    ...
    - Faithful (`bindResult`, the identity on `BVal.value`): binds `result := 2^64 − 1`,
      so `ens : result < 0` is false, and the wrong contract is refuted.
    - Signed mis-read (the bug `bindResult` must not commit): reinterprets the top bit,
      binding `result := −1`, so `ens : result < 0` discharges true, and a wrong
      contract certifies.

  `2^64 − 1 ↦ −1` is the signed reinterpretation the §4.1.1 discipline forbids
...
/-- The body converges to the `u64`-rim value `2^64 − 1`: the witness is real,
...
/-- The signed mis-bridge certifies a wrong contract. The signed mis-read
...
/-- The faithful bridge refutes the same contract. `bindResult` binds the identity
    `result := 2^64 − 1`, so `ens : result < 0` denotes false, and the wrong contract does
...
/-- The divergence in one statement: at the same witness the signed mis-bridge and the
    ...
    the identity-on-`BVal.value` bridge keeps the exec-body obligation faithful. -/
