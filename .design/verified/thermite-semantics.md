...
<!--
tier: 3-component
status: committed-architecture (prover + vocabulary + architecture LOCKED; increments unbuilt)
...
increment-(a)-blocker: #170 (spec-sublanguage S_C + prove ref_contract_pred sound, in Lean — NEXT BUILD)
increment-(b)-blocker: #171 (exec-expression S_E + prove exec_ref_value sound, in Lean)
increment-(c)-blocker: #172 (exec-statement S_B + prove body_ref_state sound, in Lean)
increment-(d)-blocker: #174 (compose → whole-frozen-subset semantic-preservation theorem, in Lean)
...
The lowering-soundness arc (epic #169) has a single open link: the production lowerer
...
| 4 | Coq itself | **the Lean 4 kernel** + (today) Z3/Verus, with the Lean-SMT cvc5-reconstruction path the route to demote Z3 (finding #8) |
...
- **REQ-4 (the increment roadmap — Lean-targeted, honest, multi-cycle)** — the spec-first ordering
  ...
  #170, is the NEXT BUILD.** Honest magnitude: CompCert/seL4-class.
...
We cannot fully MECHANIZE (T1) yet (the Lean project is increment (a), #170, the next build). But
the obligation is concrete and non-trivial. Two cases.
...
| (b) | exec-expression `S_E` + prove `exec_ref_value` sound | `S_E` | `exec_ref_value` | **#171** | bounded-value denotation; the #122/#146 exec-class; reuses the (a) Lean project. |
...
architecture). The honest magnitude: this is CompCert/seL4-class effort. CompCert took
...
- This is the soundness ARC's keystone, not its completion. The keystone makes the arch STATEABLE +
  load-bearing; the increments (#170/#171/#172/#174) build it, in Lean.

## REQ status

| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (the semantics S — denotation over the frozen subset, mechanized in Lean 4) | NOT-STARTED | open blocker #170 (S_C) + #171 (S_E) + #172 (S_B). `S` is stated as inference rules ABOVE (Architecture), GROUNDED in the SHIPPED encoders' admitted-node sets (`ref_contract_pred`/`exec_ref_value`/`body_ref_state` in `thermite-tv/src/{ref_encode,exec_encode,exec_stmt_encode}.rs`) and the frozen `lookup(C).verus_l3` (`thermite-spec/src/combinators.rs`) and UNIFIES the EXISTING state-transformer from `exec-stmt-tv.md` REQ-2. The Lean mechanization target is now PINNED (REQ-6: `lean/Thermite/{Ast,Denote}.lean`) but the formal artifact is unbuilt — increment (a), #170, the next build. |
| REQ-2 (T1 — verified-validator / reference-encoder soundness w.r.t. S) | NOT-STARTED (the FULL `S_C` `∀ P`; the comparison/logical FRAGMENT is kernel-checked under #170, the ARITHMETIC + CAST FRAGMENT under #176/#177) | blocker #170 (the FULL S_C proof — incl. combinators, spec-fn calls, the slice→`@`/byte-view/method rewrites, match/is — remains open). (T1) `∀ P, ⟦R(P)⟧ = ⟦P⟧_S` is STATED precisely above + written out CONCRETELY for two cases (AC-2). **Proved-so-far:** `theorem ref_sound (e : Expr) (env : Env) : refDenote e env ↔ denote e env` in `lean/Thermite/Soundness.lean` — kernel-checked by structural induction, NON-VACUOUS (`refDenote`/`refIntVal` via the encoder's maps vs `denote`/`intVal`'s source meaning, defined in separate modules), over: (i) **#170** the comparison/logical fragment (`Eq/Ne/Lt/Le/Gt/Ge/And/Or/Not`; the negative `eq_le_infidelity_breaks_soundness` shows an `Eq→<=` map BREAKS soundness — the boss's `==`-vs-`<=` teeth); (ii) **#176** the ARITHMETIC operators (`Add/Sub/Mul/Div/Rem/Shl/Shr/BitAnd/BitOr/BitXor`) over the unbounded-`int` spec domain (NO wraparound — overflow is the exec-side #171 obligation), via the shared `arithDenote` + the `encArith`/`tokArith` operator-map round-trip (`tokArith_encArith`); the PARTIALITY of `Div/Rem/Shl/Shr` (zero divisor/shift) is a SOURCE precondition (L0), modelled with Lean's TOTAL `Int` ops under the divisor-≠0 convention, held CONSISTENT between `denote` and `refDenote` (so T1 is insensitive to the partial point); (iii) **#177** the CASTS (`as u64/u32/usize/nat/int`) via the shared `castDenote` + the `encCast`/`tokCast` cast-target round-trip (`tokCast_encCast`), `as nat` = `Int.toNat` under a `≥0` source frame. **#122/#146 RETIRED on the contract side:** the negative `cast_paren_drop_breaks_soundness` proves a paren-DROPPED encoder (`(n-1) as nat` → `n - 1 as nat` re-parsing as `n - (1 as nat)`) DISAGREES with the faithful denotation at `n=-1` (`0 ≠ -2`) — the faithful `encode_cast` paren is exactly what makes T1 hold for casts. `#print axioms ref_sound → [propext]` (no `sorryAx`). The remaining `S_C` constructs (the 8 combinators, spec-fn calls, method/slice→`@`/`spec_*` byte-view rewrites, match/is) are the #178+ sub-increments, NOT yet embedded (no `sorry`). |
| REQ-3 (T2 — semantic preservation, as forward simulation) | NOT-STARTED | open blocker #174 (compose). (T2) `[Z3 ⊢ lower(P) ⟺ R(P)] ∧ (T1) ⟹ ⟦lower(P)⟧ = ⟦P⟧_S` is STATED in forward-simulation vocabulary (the relation `∼`, observable = caged fragment + `fx` rows; Leroy/CompCert finding #2) with its one-line modus-ponens proof + its `{Z3, S}` relativity + the reduced-trusted-base coverage boundary (AC-3/AC-4, finding #3). Contingent on (T1) (#170/#171/#172). Unbuilt because (T1) is unbuilt. |
| REQ-4 (the increment roadmap — Lean-targeted, honest, multi-cycle) | SHIPPED | the spec-first ordering table above ((a) #170 → (b) #171 → (c) #172 → (d) #174), all four now Lean 4 proofs, with per-increment rationale + the honest-magnitude statement (≈ one-pass effort, Necula finding #4). The plan is authored + the blockers are filed (#170–#174 all open). Non-doc consumer: the five filed blockers reference back to this doc; increment (a) #170 is teed up as the next build. |
| REQ-5 (the tooling decision — Lean 4 / Mathlib / Lean-SMT, COMMITTED) | SHIPPED | the decision is RECORDED above with its TCB-shrink rationale (Lean-SMT cvc5 reconstruction = the Z3-demotion path, finding #8) and Coq / Isabelle / Verus-native recorded as considered-and-deferred with one-line reasons. Blocker #173 RESOLVED (the human's call, per the #173 result comment). The DECISION is taken; this is no longer an open fork. |
| REQ-6 (the Lean project setup — pinned for the builder) | SHIPPED (for the comparison/logical fragment; increment (a) opening move, #170) | The `lean/` project EXISTS and `lake build` kernel-checks clean (NO `sorry`/`axiom`/`admit`/`native_decide`). Files: `lean/lean-toolchain` (`leanprover/lean4:v4.30.0`), `lean/lakefile.toml` (library `Thermite`), `lean/Thermite.lean` (root) + the four modules `Thermite/Ast.lean` (the `inductive Expr` for the comparison/logical fragment — `intLit`/`boolLit`/`var`/`cmp`/`logic`/`neg`, mirroring the `BinOp::{Eq,Ne,Lt,Le,Gt,Ge,And,Or}`/`UnaryOp::Not` arms of `thermite-syntax/src/ast.rs`), `Thermite/Denote.lean` (`def denote`/`def intVal` — the source `S_C` meaning), `Thermite/RefEncode.lean` (`def refDenote` via `encOp`/`encLog` mirroring `ref_encode.rs::binop_str`), `Thermite/Soundness.lean` (`theorem ref_sound`/`ref_sound_eq` + the negative `eq_le_infidelity_breaks_soundness` + — #176/#177 — `tokArith_encArith`/`tokCast_encCast`/`refIntVal_eq_intVal` and the cast-paren negative `cast_paren_drop_breaks_soundness`). **#176/#177 EXTENSION:** `Ast.lean` += `inductive ArithOp` (the 10 arithmetic ops) + `inductive CastTy` (`u64/u32/usize/nat/int`) + the `Expr.arith`/`Expr.cast` constructors (mirroring `BinOp::{Add..BitXor}` + `Expr::Cast`/`Type`/`PrimType`); `Denote.lean` += `def arithDenote`/`def castDenote` (the SHARED int-meaning/coercion, routed through by both denotations) and `intVal`'s `arith`/`cast` arms; `RefEncode.lean` += `encArith`/`encCast` (mirroring `ref_encode.rs::binop_str` arithmetic arms + `cast_target`) + `tokArith`/`tokCast` + `refIntVal`'s `arith`/`cast` arms (FAITHFUL to `encode_binary`'s whole-binary paren + `encode_cast`'s `({inner}) as {target}` paren — the #122 discipline). The Rust↔Lean correspondence gap is stated (above). **HONEST DEVIATION from this REQ's dependency list:** still Lean 4 CORE ONLY (no Mathlib, no Lean-SMT) — the arithmetic+cast fragment is provable with core `Int`/`Nat`/`Bool`/`Prop` + `simp`/`rfl`/`cases`/`induction` (the bitwise/shift ops use core `Nat.land`/`Nat.shiftLeft`/… over `Int.toNat`, no Mathlib needed); Mathlib/Lean-SMT are added when a later increment's proof genuinely needs them. The remaining deferred AST constructs (the 8 combinators incl. recursive/quantified bodies, named spec-fn calls, method/slice→`@`/`spec_*` byte-view rewrites, `Match`/`Is`) are listed in `lean/Thermite/Ast.lean` as the #178+ sub-increments — NOT embedded-then-`sorry` (forbidden), simply left out of the inductive. |
