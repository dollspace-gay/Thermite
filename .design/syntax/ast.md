...
| REQ-6 — RAW (`IntLit` verbatim raw on the Expr, #37) | NOT-STARTED | crosslink #37. `IntLit` is still value-only `IntLit(u128)` (`ast.rs`); the parser drops the token's verbatim slice (`Expr::IntLit(v)` in `parser.rs`). Needs reshape to `IntLit { value, raw }` so `1_000_000` parses to `{ value: 1000000, raw: "1_000_000" }` (AC-1b), with the full consumer ripple updated and lowering still emitting `value` (no golden churn). |
