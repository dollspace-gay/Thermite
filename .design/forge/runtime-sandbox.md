...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (seccomp prelude install via raw `libc` `prctl`) | NOT-STARTED | open prereq blocker #57. `forge/src/sandbox.rs` does not exist; the `--entry` generated `main` (`build::synthesize_entry_main`) currently emits no prelude (`build.rs` REQ-6 row: "v0.1 installs no sandbox"). Mechanism grounded (probe killed, exit 159). |
| REQ-2 (transitive-`fx` derivation via `closure.rs`) | NOT-STARTED | open prereq blocker #57. The seam `closure::reachable_in_file_fns` + `manifest::effects_of` exist and are SHIPPED, but no consumer unions them into a sandbox allowlist yet. |
| REQ-3 (`fx` → syscall allowlist mapping) | NOT-STARTED | open prereq blocker #57. The mapping table is grounded (pure baseline runs clean, `openat` denied/allowed splits on `read`) but unimplemented; no `fx_to_syscalls` fn exists. |
| REQ-4 (sandbox-on-by-default for `--entry`, `--no-sandbox` opt-out) | NOT-STARTED | open prereq blocker #57. `cli`/`build_file` take no sandbox flag; `forge build --entry` produces an unsandboxed exe today. |
| REQ-5 (reproducible prelude + manifest record) | NOT-STARTED | open prereq blocker #57. `BuildManifest` carries the `fx` rows (#56, SHIPPED) but no installed-allowlist field; no prelude bytes to make deterministic yet. |
| REQ-6 (demonstrable enforcement — probe + clean pure run) | NOT-STARTED | open prereq blocker #57. No `--sandbox-self-test` flag / probe injection exists; the enforcement is grounded only in the throwaway probe (cleaned up), not in the toolchain. |
