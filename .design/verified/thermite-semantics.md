...
| REQ-2 (T1 — verified-validator / reference-encoder soundness w.r.t. S) | NOT-STARTED (the FULL `S_C` `∀ P`; the comparison/logical FRAGMENT is now kernel-checked under #170) | blocker #170 (the FULL S_C proof — incl. combinators, spec-fn calls, the `as nat`/coercion class — remains open; this increment is its OPENING MOVE). (T1) `∀ P, ⟦R(P)⟧ = ⟦P⟧_S` is STATED precisely above + written out CONCRETELY for two cases (AC-2). **Proved-so-far:** `theorem ref_sound (e : Expr) (env : Env) : refDenote e env ↔ denote e env` in `lean/Thermite/Soundness.lean` — kernel-checked by structural induction over the comparison/logical fragment (`Eq/Ne/Lt/Le/Gt/Ge/And/Or/Not`), NON-VACUOUS (`refDenote` via the encoder's `encOp`/`encLog` operator map vs `denote`'s source relations, defined in separate modules; the negative `eq_le_infidelity_breaks_soundness` shows an `Eq→<=` map BREAKS soundness — the boss's `==`-vs-`<=` teeth at the Lean level). The remaining `S_C` constructs (arithmetic/coercions, the 8 combinators, spec-fn calls, method/byte-view rewrites, match/is) are the #171+ sub-increments, NOT yet embedded (no `sorry`). |
