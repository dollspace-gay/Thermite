...
| REQ-8 (`fx diverge` caps at L1, mutation/strengthen exempt — the #16 mirror) | NOT-STARTED | open prereq blocker #88. The row-shape predicate `fn_is_diverge in lower.rs` ships and the #87 termination exemption (`#[verifier::exec_allows_no_decreases_clause]` in `lower_fn`) lets a diverge fn prove its loop invariants (partial correctness), but `gate_fn in check.rs` has NO `fn_is_diverge` branch — a diverge fn falls through `GateOutcome::ProceedToL3`, proves L3, then the §7 mutation gate (`Certificate::rejected_weak_contract`) rejects `run` at `Level::L0` (`WeakContract`) because `ens result <= 256` is a weak shape (`return 0` survives). No diverge `GateOutcome` arm / L1 cert exists. The #16 precedent to mirror is `gate_fn`'s `f.boundary.is_some()` → `Certificate::boundary_l1`. |
