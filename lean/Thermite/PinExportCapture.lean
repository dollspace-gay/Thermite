/-
  ...
  unfolding is an unsound export". `substitute` detects a collision (a body
  binder, a closure element var or a match payload binder, that equals a free name of
  ...
  interactive path). No capture is emitted.
  ...
  shadowing direction (no live arg uses the binder name) is handled soundly by
  ...
  The captured-tautology emission this file once pinned
  ...
  reproduces the captured emission; it pins the meaning gap that motivates the
...
/-- What `unfold_once`/`substitute` would have produced for `cntk(xs, k as int)`
    ...
    so the predicate becomes the tautology `|k| k as int == k as int`. The fixed
...
/-- The probe env: `xs = [0, 1]`, caller `k = 0`, empty registry (the unfolded
    forms are specCall-free, so the registry does not matter here). -/
def envI : Env :=
  { ints := fun _ => 0
...
/-- The §6.1(b) meaning gap the capture guard walls off: the captured Expr does not
    equal the real body substituted, and their denotations differ at `envI`. A
    capture-unsafe unfolding must therefore be refused (#245), never emitted. -/
