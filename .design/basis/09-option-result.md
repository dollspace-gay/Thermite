...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (built-in `Option<T>` — `Some`/`None` construct + match + `is`) | NOT-STARTED | open prereq blocker **#95**. `Option` MATCHES today (`Pattern::Enum`, `.design/basis/01-adts.md` REQ-9 SHIPPED) but does NOT construct: `Some(v)`/`None` in expr position reach `thermite-spec/src/validator.rs`'s variant check and reject (`SpecError::UnknownVariant`, `"Some is not a declared variant"`) — `Validator::new` seeds NO built-in variants. No `Type::Option` node (`Option<usize>` is a string-named `Generic` in `enum Type`, `ast.rs`). GROUNDED-feasible (verus `4 verified, 0 errors`), not implemented. |
| REQ-2 (built-in `Result<T, E>` — type + `Ok`/`Err` construct + match + `is`) | NOT-STARTED | open prereq blocker **#95**. `Result<T,E>` does NOT PARSE: `parse_type_inner` (`parser.rs`) parses `NAME<T>` into single-arg `Type::Generic { name, arg }` and `consume(&Gt)` fails on the comma (`expected >, found ,`). No `Type::Result` two-arg node; `Ok`/`Err` are not registered built-in variants. GROUNDED-feasible (verus `3 verified, 0 errors`), not implemented. |
| REQ-3 (built-in-variant registry + spec-`match`-in-`ens` payload projection) | NOT-STARTED | open prereq blocker **#95**. `Validator::new` collects only USER `Item::Enum` variants into `enums`/`variant_to_enum` (`validator.rs`, 01-adts REQ-5/REQ-6 SHIPPED) — `Some`/`None`/`Ok`/`Err` are not seeded, so construction/`match`/`is` over them reject. The spec-`match` IS already admitted as a flat built-in (`walk_expr_inner` `Match` arm, 01-adts REQ-7 SHIPPED) — the gap is only the built-in-variant seeding. GROUNDED: spec-`match`-in-`ens` verifies `5/0`, broken-body FAILS. |
| REQ-4 (`Option`/`Result` → Verus types; construct/match/`is`/spec-match lower) | NOT-STARTED | open prereq blocker **#95**. `lower_type` (`lower.rs`) has no `Type::Option`/`Type::Result` arm; `qualify_variant_path` qualifies only user-enum variants via `enum_of_variant` (a built-in `Some`/`Ok` already falls through to the bare name Verus wants — GROUNDED — so the lowerer change is the `lower_type` arms + leaving built-ins unqualified). GROUNDED-feasible, not implemented. |
| REQ-5 (`parse_u64` — `String`→`u64`, the C4 REQ-9 payoff, ships under C7) | NOT-STARTED | open prereq blocker **#95** (depends on REQ-1..REQ-4). `.design/basis/07-strings.md` REQ-9 is NOT-STARTED pending this C7 surface. GROUNDED: the Horner loop + the three handled-or-loud `None` arms + the spec-`match`-in-`ens` round-trip verify `5 verified, 0 errors`; the broken `Some(0)` FAILS `3 verified, 1 errors`. The gap is purely the surface spelling of the partial return type + payload projection, NOT the verification. |
| REQ-6 (`LowerError`/`SpecError` extension, no panics) | NOT-STARTED | open prereq blocker **#95**. The C7 constructs reuse the existing `LowerError`/`SpecError` enums (no new variant expected — admitting the built-in variants is a registry change, not a new error mode; a genuinely undeclared variant still rejects `UnknownVariant`). Not implemented; pinned so the critic checks no `unwrap`/`panic!` lands (R-CODE-2). |
