...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (frozen mutator set) | NOT-STARTED | open prereq blocker #46. `forge/src/mutation.rs` does not exist; no mutator-set type or generator over `ast.rs` `BinOp`/`IntLit`/`Stmt::If`/`Stmt::Return` is shipped. |
| REQ-2 (deterministic order + seed + cap) | NOT-STARTED | open prereq blocker #46. No `MUTANT_CAP` const and no seeded deterministic enumeration exists. |
| REQ-3 (re-lower + re-verify vs same contract) | NOT-STARTED | open prereq blocker #46. The reuse seams ship (`thermite_lower::lower`, `check::item_subprogram`, `check::run_verus`) but no mutant-scoring caller composes them. |
| REQ-4 (KILLED vs SURVIVED) | NOT-STARTED | open prereq blocker #46. `check::classify_verus_outcome` ships but is not invoked on mutant runs; no kill/survive classification exists. |
| REQ-5 (kill ratio + 60% floor gate) | NOT-STARTED | open prereq blocker #46. No `MUTATION_FLOOR` const, no `kill_ratio` computation, no `RejectReason { cause: "WeakContract" }` producer. |
| REQ-6 (graduate `mutants_killed`/`survivor`) | NOT-STARTED | open prereq blocker #46. `manifest::ContractQuality::forward_declared` still emits `mutants_killed: "0/0"`, `survivor: None`; no `with_mutation_score`/`rejected_weak_contract` constructor exists. |
| REQ-7 (gate AFTER L3, reuse proof cache) | NOT-STARTED | open prereq blocker #46. `check::check_file`'s L3 path has no post-`Proved` mutation stage; `cache::cache_key`/`load`/`store` ship but no mutant re-verify keys through them. |
| REQ-8 (deterministic kill ratio, oracle stance) | NOT-STARTED | open prereq blocker #46. The determinism prerequisites ship (pinned seed, deterministic lower + cache) but no scorer exists; `mutants_killed` remains forward-declared in `Certificate::oracle_subset`. |
