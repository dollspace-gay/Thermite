...
| REQ-KPRIM-11 | not_started | `.design/build/kernel-primitives.md` | Source-derived machine-class gate | `.design/build/frozen-primitive-registry.md` section "Source-derived minimum machine class" defines the total map from the twelve closed platform effect atoms to a minimum class, the order sequential < volatile < atomic < privileged, the maximum rule over an entry's whole effect row, and the reporting rule for a door no schema models. Forge still gates on `entry.concurrency` alone in `fn plan_from_bytes`, so a `sequential` declaration over a `fx platform(atomic)` door publishes an end-to-end L3 artifact with zero residual machine assumptions. Implement the gate, keep the `platform(clock)` synthetic primitives passing, and turn the two pinned divergence tests green.<br>blockers: test:forge/tests/divergence_registry_v4_matrix.rs |
