/-
  ...
  - The source measure: `x - x = 0`, constant, never strictly descends: the
  ...
  and closure step to include `dec` clauses (item's `dec` plus each reached
...
/-- Against the true registry the measure denotes `x - x = 0` at every
    resolving fuel: the source measure is the constant 0. -/
...
/-- Uniqueness (the doc's own #214 lever) forces that value. -/
...
/-- The pin, direction 2: the source measure never strictly descends
    (constant 0 → 0); it is the non-well-founded measure of a
