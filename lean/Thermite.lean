...
-- vendored cvc5 1.3.2; the hand-translation residual; Verus/Z3 not emitting
-- reconstructable certificates) are in
...
-- `(P_prod) ⟺ (P_ref)` Lean goals: one QF_LIA scalar clause and three literal
-- `BitVec N` QF_BV clauses, including the full arithmetic/bitwise/shift/division term
-- surface. QF_LIA uses `smt`; QF_BV uses kernel-checked normalization lemmas.
-- `#print axioms` stays within {propext, Classical.choice, Quot.sound}. The file is
-- generated verbatim and pinned by `golden_file_matches_exporter`.
import Thermite.SmtExport
-- Layer 4 reference theorem (#356): the older bounded-integer model remains proved
-- equivalent to `BitVec N`. The active exporter now emits literal `BitVec N`, so this
-- module is a supplementary semantic cross-check rather than a bridge in the export path.
