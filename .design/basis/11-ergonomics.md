...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (tuple destructuring `let (x,y)=e`) | NOT-STARTED | blocker #115. No `Pattern::Tuple` in `ast.rs`; `parse_let` (`parser.rs`) accepts only an `Ident` name, no pattern LHS. The desugar target `Expr::TupleProj` IS shipped (`lower_expr` in `lower.rs`); only the parse → desugar is missing. GROUNDED feasible: the `t.0`/`t.1` desugar verifies L3 (Verification AC-1). |
| REQ-2 (`for i in 0..n` loop) | NOT-STARTED | blocker #118. No `for`/`in` keyword (`keyword_kind` in `lexer.rs` has `loop`/`while`/`match` only), no `Stmt::For`. The desugar target `LoopNode`(`While`) + `inv`/`dec` threading IS shipped (`lower_loop` in `lower.rs`). GROUNDED feasible: the for→while desugar with AUTO-`dec n-i` verifies L3 and a bad inv is L0 (Verification AC-2/AC-2b). |
| REQ-3 (match guards `x if cond =>`) | NOT-STARTED | blocker #115. `MatchArm` (`ast.rs`) has `{ pattern, body }` — no `guard` field; `parse_match` (`parser.rs`) builds `MatchArm { pattern, body }` with no `if` clause; `lower_match` (`lower.rs`) emits `pat => body` with no guard; `check_match_exhaustiveness` (`validator.rs`) does not down-weight a guarded arm. GROUNDED feasible: a guarded match verifies L3 and a guarded-only `Some` arm is rejected as non-exhaustive (Verification AC-3/AC-3b). |
| REQ-4 (or-patterns `1 \| 2 =>`) | NOT-STARTED | blocker #118. No `Pattern::Or` variant (`enum Pattern` in `ast.rs`); `parse_pattern_inner` (`parser.rs`) has no `\|` alternation; `lower_pattern` (`lower.rs`) has no `Or` arm; `check_match_exhaustiveness` (`validator.rs`) counts one variant per arm, not an or-pattern's union. GROUNDED feasible: an or-pattern verifies L3 and `Some(_) \| None` is exhaustive (Verification AC-4). |
| REQ-5 (`if let` / `while let`) | NOT-STARTED | blocker #119. No `if let`/`while let` form (`parse_if_parts`/`parse_loop` in `parser.rs` take a bare cond `Expr`, no `let`-binding head); no `Stmt::IfLet`/`WhileLet`. The desugar targets `Expr::Match` (`lower_match`) + `LoopNode`(`While`) + `Expr::Is` ARE shipped. GROUNDED feasible: the `if let`→`match` and `while let`→`while (e is Variant)` desugars verify L3, and the rejected `loop+break` while-let alternative is L0 (Verification AC-5/AC-6). |
