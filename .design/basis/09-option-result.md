...
| REQ-5 (`parse_u64` — `String`→`u64`, the C4 REQ-9 payoff, ships under C7) | SHIPPED | #95 C7. `lower.rs::emit_parse_defs` emits the `is_digit`/`all_digits`/`parse_be` spec fns + the `parse_u64(s: &TString) -> Option<u64>` exec fn (the Horner-accumulate loop `acc = acc*10 + digit`, the BE partial-value invariant `parse_be(s.data@.subrange(0, i)) == acc` + the all-digits prefix witness + `decreases s.data.len() - i`, and the three handled-or-loud `None` arms — empty / non-digit / overflow, each screaming BEFORE corrupting `acc`) with the round-trip success `ens match result { Some(v) => all_digits(s.data@) && s.data.len() >= 1 && parse_be(s.data@) == v as nat, None => true }`. Materialized when `program_uses_parse` (a `parse_u64` call); `parse_be` deduped against the numfmt emission. NO `assume`/`external_body`/`admit` (R-DEFER-9). Consumer: `lower`. Verified: `forge/tests/option_result_conformance.rs::ac4_parse_u64_lowering_verifies_under_real_verus` (real verus `5 verified, 0 errors`) + `ac4_broken_parse_u64_body_fails_real_verus` (broken `Some(0)` FAILS, non-vacuous). |
