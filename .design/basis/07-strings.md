...
tier: 3-component
status: draft
audited-sha: cf48c8931ae9c8a5ca37a0ba4225f8298fc7d527 (re-pinned 2026-06-12: #279 — the bytes_eq field-access operand byte-view fix touched lower.rs at cf48c8931ae9c8a5ca37a0ba4225f8298fc7d527; whole-operand-class closure, claim-neutral to this doc's REQs)  (prior: af1e6f2c71b8da74f0bedde4681e4ed62f3836ef (re-pinned at the #278 C8 build HEAD — the governed files lower.rs/validator.rs/l1.rs last touched there; REQ-17..REQ-20 SHIPPED)  (prior: fe75e80aaa5cc32d924f1485bad472abc8684995 — the #269/#270 coordinated arc — the touched-file changes are exactly this doc's designed REQs / reviewed as claim-neutral)  (re-audited 2026-06-12: amended — REQ-1..5 evidence cells re-grounded in the shipped tree (they carried pre-build gap text under SHIPPED status), C5/REQ-9 Summary clauses updated to shipped, REQ-4 narrowing claim updated per #225, #262)  (re-pinned 2026-06-12: #277 — REQ-4 slice/concat ens STRENGTHENED from length-only to BYTE-CONTENT (slice subrange, concat append); the #276 prerequisite, emitted bodies + both goldens re-verify, prose + table row updated)  (re-pinned 2026-06-12: #278 — cluster C8 DESIGNED + GROUNDED: `bytes_eq` as a REGISTERED built-in spec predicate + the prove-once bridge-lemma family the #276 Arc-2 STOP requires; REQ-17..REQ-20 NOT-STARTED under #278)  (re-pinned 2026-06-12: #278 SHIPPED — REQ-17..REQ-20 flipped to SHIPPED: `bytes_eq` joins `GENERATED_SPEC_FNS` (validator.rs), `emit_bytes_eq_defs` materializes the low-peel def + four prove-once lemmas (lower.rs), the contract-keyed `lemma_bytes_eq_bridge()` body-start citation, the L1 exec twin (l1.rs); the emitted `conformance/bytes_eq_demo.th` lowering verifies `17 verified, 0 errors` through the REAL pipeline + golden `tests/golden/lower/bytes_eq_demo.verus.rs` + cert `conformance/bytes_eq_demo.cert.json`, the head/tail-swap mutant `16 verified, 1 errors` (non-vacuous), skill budget held 5988/6000; ONE divergence — the `lemma_bytes_eq_bridge` body fill added an explicit per-index subrange `#[trigger]` the signature-only pin did not spell))
