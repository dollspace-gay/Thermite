...
audited-content-sha256: a5f181049878da972642c9681c12afc73abdb1f118ff7acc9d2f76d536aee873 (re-pinned 2026-08-07 after source-oriented Forge commands resolved canonical packages through one shared front door; existing single-file behavior remains regression-covered)
...
only the frozen contract sublanguage (comparisons, logical connectives, `result`/`old`/`final`, named
...
- **REQ-1 (independent reference encoder)** — `thermite-tv::ref_encode::ref_contract_pred(clause.expr, &RefCtx) -> Result<VerusPredicate>` maps a SpecTherm contract clause's `Expr` (`thermite-syntax::ast::Expr`) to a Verus predicate STRING, covering exactly the contract sublanguage (`thermite-design.md` §4.2): comparisons (`Expr::Binary` with `BinOp::Eq|Ne|Lt|Le|Gt|Ge`), logical connectives (`And`/`Or`/`Unary::Not`), arithmetic in subterms, `result` plus `old(x)`/`final(x)` state references, structural field projection and explicit snapshot dereference, named `spec fn` calls (`Expr::Call`), the 8 frozen combinators (resolved via `thermite_spec::lookup(name).verus_l3`), fixed-array `.array_eq(other)` and `.array_same_except(other, index)` relations over scalar or finite plain aggregate elements, total `u64` bit operations/preservation witnesses, and the spec-context slice/string/map/cast rewrites. Forge supplies exact reachable record/aggregate declarations to the independent obligation frame but does not import production lowering or comparator logic. Derived from `thermite-design.md` §4.2. **HARD CONSTRAINT (R-CHAR-3 / the trust model):** it MUST NOT call `thermite_lower::lower::lower_expr` or any production lowering symbol — independence is the entire point.
- **REQ-2 (per-clause Z3 equivalence obligation + discharge path)** — `thermite-tv::obligation::equivalence_obligation` emits a `proof fn tv_<clause-addr>(<free vars + result + state snapshots>) requires <enclosing req> { assert(P_production <==> P_reference); }` Verus unit, with the clause's spec-fn deps + the referenced combinators' `verus_l3` defs in scope. `old(param)` and `final(param)` are reified as distinct arbitrary snapshot parameters because Verus state operators are unavailable in a proof fn; named-record snapshots retain shared-reference type so both implicit field projection and `*old(...)`/`*final(...)` remain well-typed. This checks every possible transition and does not collapse the proof to a no-op post-state. Only exact canonical production state-view calls are reified, so a wrong operator, argument, or view remains divergent or ill-typed. It is discharged through the existing Verus path; VERIFIED ⟺ faithful, a counterexample ⟺ infidelity. Derived from `thermite-design.md` §6 (L3 = the verus-derived SMT proof). The production side remains the actual lowered clause text (the artifact under test); the reference side is REQ-1.
...
- **AC-8 (arbitrary post-state)** — a faithful `final(data)[i]` clause over `data: &mut [[u64; N]]` verifies when `final(data)` is bound as an independent sequence snapshot; replacing it with the current/pre-state view fails the equivalence assertion.
...
referenced slice/scalar/named params) + `result` (when the return is non-unit) + `old(x)`/`final(x)` values
bound as distinct arbitrary snapshot params (shared snapshot references for named records), under the enclosing `req` as a `requires`, and the clause's spec-fn + combinator
