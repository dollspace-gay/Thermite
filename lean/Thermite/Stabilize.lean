/-
  ...
  fuel-indexed `Denote.lean` denotation rather than a raw fuel index, and the result value
  is bound through that relation. This module lands the named Lean additions §4 says
  increment (ii) needs in the spine before the exporter can target the form:
    ...
    - the bottom-distinguishing none-propagating denotation `intValNB`/`denoteNB` (+ the
      ...
      `Denote.lean`'s recursion save three points: a fuel-0 `specCall` → `none`; an unresolved
      `specCall` → `none`; every arm propagates `none`. `intValNB f e env = some v` means
      `e` reached a value without bottoming — what the bottoming `intVal` cannot
    ...
    - `RegistryTerminating env e := ∃ v, Converges e env v` (the #241 fix, replacing the
      ...
      `stabilization_exists_for_dec_bounded`) carrying the agreement lemma's content.
      The divergent registry `f(x)=f(x)` now fails the hypothesis
  ...
  untouched and stay green (they import `Thermite.Denote`, not this module).
-/
...
`fuel₀`, which is what fixes the value-dependent-depth counterexample of #213
...
is well-defined because `stabilizes e env v` pins one `v`. The argument is
...
construction), so it traverses the same positions `intVal`/`denote`/`seqVal`/
...
mutual
/-- `e : Expr` contains no `specCall` (over the full AST). `false` at a `specCall`
    node and propagated through every sub-position. -/
def specCallFree : Expr → Bool
  | Expr.intLit _ => true
  | Expr.boolLit _ => true
  | Expr.var _ => true
  -- The #253 bool-var: a free name, no subterms, so spec-call-free (`true`).
  | Expr.boolVar _ => true
  | Expr.seqVar _ => true
  | Expr.strVar _ => true
...
recursion is structural (this is the cycle-7 critic's "only the two specCall arms
...
theorem denote_fuel_irrelevant : ∀ (e : Expr) (env : Env) (f g : Nat),
  ...
  -- The #253 bool-var: denotes `env.bools x = true` (fuel-free), so the iff is reflexive.
...
fuel-free shallow statement `denote 0 e env` / `intVal 0 e env = v` — the QF shape
the z3-demotion PoC discharges (§6 tier (a)). -/

/-- Tier-(a) key (Prop side): for spec-call-free `e`, `stabilizesProp e env ↔ denote 0 e env`.
    The fuel-free shallow statement the auto battery consumes. -/
...
/-- Tier-(a) key (int side): for spec-call-free `e`, `stabilizes e env v ↔ intVal 0 e env = v`.
    The fuel-free shallow value the auto battery consumes. -/
...
`f7d288ef`). The bottoming `intVal` cannot distinguish a genuine stabilized value from a
divergence stuck at the bottom.
...
`Denote.lean`'s recursion save three points: a fuel-0 `specCall` yields `none`; an
...
reached a value at fuel `f` without hitting a bottom arm, which the
...
`Converges → stabilizes`, giving the §4 supporting lemma its content. The divergent
...
/-- The none-propagating count for the `countWhere` combinator (the #182 arm): mirrors
    ...
    Structural recursion on the slice (core, no fuel, as in `countWhereVal`). -/
...
/-- The per-element predicate gate for the six Prop-combinators (the #242 root fix): given a
    ...
    recursion on the slice (core, no fuel, as in `countWhereValNB`). -/
...
/-- When every element's predicate is `some`, the gate is `some ()` (the converse direction:
    ...
    comb arm supplies `hp`, as `countWhereValNB_eq_of` is supplied). -/
...
/-- The none-propagating sequence denotation (the `seqVal` mirror): `some s` iff `e`
    ...
    and it is a genuine value, never a bottom. -/
...
/-- The none-propagating int denotation (the `intVal` mirror, the #241 root). `some v` iff
    `e` reaches an integer value at fuel `f` with no bottom arm taken. The three
    ...
    (`intLit`/`var`) and the bool-sorted catch-all (`some 0`) match `intVal`'s values;
    only the `specCall` bottoms become `none`. -/
...
/-- The none-propagating Prop denotation (the `denote` mirror): `some P` iff `e` reaches a
    genuine proposition at fuel `f` with no bottom arm; the carried `P` is the
    ...
    propagates `none`. The 6 bounded combinators + permutationOf mirror `denote`
    (the inline per-element predicate `p` is `denote`'s own, so agreement is reflexivity),
...
noncomputable def denoteNB : Nat → Expr → Env → Option Prop
  ...
  -- each slice element (`env.bindInt bound x`) — the positions the spine `denote`'s
  ...
  -- `idx`, as before). The carried proposition is still the spine `denote` form: the gate
  -- decides `some`/`none`, never which proposition is carried, so `denoteNB_agrees`
...
/-! ## The agreement lemmas (the content #241 demands)
...
divergent registry has no `some` witness, so the agreement is vacuous there. -/
...
theorem denoteNB_agrees : ∀ (f : Nat) (e : Expr) (env : Env) (P : Prop),
  ...
  | f, Expr.comb c seq seq2 idx pred, env, P, h => by
      -- The comb NB arm carries the spine `denote` proposition once the slice/`seq2`/
...
sufficiently large fuel: `e` computes `v` per env, never bottoming. Unlike
...
lemma `converges_imp_stabilizes` carries `Converges`'s content into `stabilizes`. -/
...
/-- The agreement lemma (the #241 content): convergence implies stabilization. When
    the none-propagating denotation yields `some v` at all large fuel, the bottoming
    `intVal` yields the same `v` there (`intValNB_agrees`), so `e` stabilizes to
...
/-- `RegistryTerminating env e` (the discharge target of the registry-termination class,
    ...
    divergent registry cleared at the Int-bottom `0`; it is the convergence the
...
/-- `stabilization_exists` (the design's `stabilization_exists_for_dec_bounded`), now with
    ...
    premise is inhabited by exactly one `r`. -/
