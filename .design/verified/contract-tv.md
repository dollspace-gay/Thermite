...
audited-content-sha256: 3fb7fcc62b8d9c1708931fd3cd2b6df84571cc6b89dd43ba3c502de6d3c47752
...
referenced slice/scalar params) + `result` (when the return is non-unit) + `old(x)`/`final(x)` values
bound as distinct arbitrary snapshot params, under the enclosing `req` as a `requires`, and the clause's spec-fn + combinator
