...
- `forge/src/body_tv.rs` — the NEW forge check phase (REQ-5, blocker #162), sibling to `exec_tv.rs`.
  Four-way `Faithful` / `Divergent` / `Unverifiable` / `Skipped` (an out-of-v1 loop body / unmodeled aggregate state / an
...
- **AC-6 (independence is structural)** — `thermite-tv` keeps NO `thermite-lower` dependency;
  ...
  thermite-tv` = syntax + spec only).
- **AC-7 (loops / out-of-scope bodies skipped HONESTLY)** — a body containing an out-of-v1 `Stmt::Loop`,
  an aggregate mutation outside the fixed mutable-slice or exact owned named-aggregate
  `FixedArray8.set` subsets, or a mid-body early return reaches the `forge::body_tv` phase as `Skipped` (with
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (frozen kernel exec-statement subset v1) | SHIPPED | the IN/OUT construct set is PINNED IN CODE: `thermite_tv::exec_stmt_encode::body_ref_state` (and its `thread_stmt`/`encode_value`/`aggregate_mutation_reference`) ADMIT `Stmt::Let`/bare-scalar `Assign`/top-level single-index assignment to a frame-declared mutable slice/`If`/`Expr`/tail-`Return` + `Block` sequencing/tail, plus the exact owned aggregate form `let mut updated: S = base; updated.arr.set(i,v); updated.field = …; updated`. The slice-effect subset requires indices/values not to read a mutable slice and rejects nested slice writes; the aggregate subset requires a named result frame, exact declared fields, and eight-slot `FixedArray8` storage. It HONESTLY REJECTS (an `Unsupported` `Err`) `Stmt::Loop`/`Break`/`Continue` (2.2.2), a mid-`if`-branch early return, `match`-stmt, any other aggregate mutation, and a re-shadow. Verified by B1–B4, honest-skip tests, the real `service_write_user_byte` body-TV pass, mutable-slice teeth, and live fixed-array faithful/wrong-index/wrong-value teeth. |
| REQ-2 (operational-semantics reference state-denotation) | SHIPPED | `pub fn body_ref_state` + `body_ref_state_ensures`/`slice_write_ensures`/`aggregate_mutation_reference`/`BodyRefCtx` in `thermite-tv/src/exec_stmt_encode.rs` — the big-step state-transformer (let/assign substitution-threading, mutation-ORDER sensitivity, `if`-branch composition, multi-cell tuple projection), a complete mutable-slice sequence transition, and exact per-field/per-slot aggregate reconstruction, composing step-2.1's `exec_ref_value` on each env-substituted value. Non-test consumer: `thermite_tv::obligation::body_equivalence_obligation`. Independence is STRUCTURAL: deps `thermite-syntax` + `thermite-spec` ONLY (`cargo tree -p thermite-tv` — no `thermite-lower`, AC-6). Lean correspondence is `Stmt.sliceAssign`/`State.setSlice` with `refStmt_eq_stmtDenote` plus `Exec/KernelAggregate.lean`'s `fixed_array_set_ref_sound`/`aggregate_set_ref_sound`; wrong-index and wrong-value theorems bite in both layers. |
