...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (independent reference encoder) | NOT-STARTED | open prereq blocker #140. No `thermite-tv` crate / `ref_encode.rs` exists; the contract-sublanguage reference encoder (independent of `lower_expr in lower.rs`) is unwritten. The frozen ground truth it reuses (`thermite_spec::CombinatorSig.verus_l3` in `combinators.rs`) and the rewrites it must re-implement (`lower_expr in lower.rs` spec-context arms: slice→@, method→spec_*, cast→nat) both EXIST as the reference points. |
| REQ-2 (per-clause Z3 equivalence obligation + discharge) | NOT-STARTED | open prereq blocker #141. The discharge path EXISTS (`run_verus`/`item_subprogram in check.rs`); the obligation builder (`thermite-tv::obligation`) does not. Feasibility GROUNDED: a hand-written `proof fn { assert(P_production <==> P_reference) }` with free vars + result/old discharges (AC-1/AC-5 above, real verus `verified, 0 errors`). |
| REQ-3 (off-corpus generator) | NOT-STARTED | open prereq blocker #142. No clause generator exists. This is the corpus-bound escape; v1 may ground on the corpus clauses + the teeth-test (REQ-4) while the generator is built — but REQ-3 is the REQ that delivers the thesis payoff (un-bounding fidelity checking). |
| REQ-4 (the teeth — R-CHAR-3) | NOT-STARTED | open prereq blocker #143. No conformance test exists. The teeth are GROUNDED against real verus (AC-2/AC-3/AC-4 above: ==→<= , wrong combinator predicate, #127-class wrong byte-view rewrite each produce a counterexample; the faithful clause verifies) — the test mechanizes those grounded runs through `forge::check::run_verus`. |
