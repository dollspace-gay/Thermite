...
A passing audit means trust reduces to a small *named* set — `{ Z3/Verus soundness, the Thermite→Verus lowering }` — and every other step reproduces on the auditor's own machine. (See [`scripts/audit.sh`](./scripts/audit.sh).)
...
- ✅ **Forge** (`forge`) — `check` (parse → validate → effect-check → lower → Verus, per-item certificate with content-addressed proof caching), `build` (→ `rustc` → a runnable binary whose contract checks fire at runtime, with an **fx-derived seccomp sandbox**), `audit` (the trust manifest + enumerable TCB), `review` (pluggable spec-intent slot), `repair` (background L1/L2 → L3 upgrades), and the **translation-validation phases** — `tv` (contracts), `exec-tv` (exec expressions), `body-tv` (straight-line bodies + v1 `while`-loop obligations), each four-way `Faithful`/`Divergent`/`Unverifiable`/`Skipped` with skips honest and divergences loud. Automatic **L3 → L2 → L1 degrade**; a counterexample never degrades and is never "repaired."
...
- 🔭 Deferred (tracked): the freestanding `no_std` kernel target (#164, #21); direct MIR-level lowering + the incremental goal-state REPL (crosslink #21); the contract-TV parallel seam (#166); the basis v1.1 layer (dataflow taint-propagation, `Vec` element-invariants, scheme fusion); and the proof-spine residuals, each named in its doc — full Z3 demotion (upstream-gated: Lean-SMT's QF_BV `sorry`, no Verus certificates), the Lean→Rust extraction bridge, user-ADT `match`/`is` in the proven fragment.
