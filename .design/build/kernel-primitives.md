...
Final finite-record constructors and exact record access paths now materialize
results leafwise after a descendant sequence overlay: scalar leaves compare
directly and array leaves compare by complete logical view without inventing a
`Seq<T>`-to-`[T; N]` conversion. Explicitly typed finite-record constructor and
access-path locals now snapshot and rebind that logical state; their scalar and
terminal indexed leaves may be mutated before a leafwise result is returned.
One direct source-derived pure function may now consume that logical finite
record by value when its other formals are scalar/unit: body TV snapshots the
formal leafwise and interprets the exact reachable Thermite body. Scalar/unit
results return directly; an admitted finite-record result with an exact
constructor/access-path tail is rebound leafwise into a typed local or direct
body tail. Generated code executes the ordinary native-record call. The
expanded 141-row fixture executes the scalar and record results from a
downstream consumer at L3.
Static ownership, index-then-field aliasing, array-element-root calls, arbitrary
native aggregate materialization, aggregate results outside that exact tail
subset, nested/general by-value call use after a descendant sequence overlay, native aggregate
matching, nested or
...
the quantified logical-index aggregate-state framing specified in
`.design/build/aggregate-array-relations.md`, and generic library
capacities/types remain.
...
contract without exposing a field. It is unimplemented under REQ-AGGREL-2
through REQ-AGGREL-5; see `.design/build/aggregate-array-relations.md`. The packed
...
One shared front door resolves every source-oriented command's path argument. A
single `.th` file keeps its existing read-and-parse behavior; a canonical
manifest resolves the declared module closure and parses each module on its own.
`check`, `audit`, `tv`, `exec-tv`, `body-tv`, `goal`, `battery`, `edit`, `fill`,
`repair`, `build --level l1`, and `smt-export` therefore accept a package. A
module syntax error names that module's package-relative path and a span inside
that module, and `edit`/`fill` splice into the file that declares the addressed
item while the manifest and every sibling module stay byte-identical. `forge
review` is the remaining single-file verb.
...
| REQ-KPRIM-11 | not_started | `.design/build/kernel-primitives.md` | Source-derived machine-class gate | `.design/build/frozen-primitive-registry.md` section "Source-derived minimum machine class" defines the total map from the twelve closed platform effect atoms to a minimum class, the order sequential < volatile < atomic < privileged, the maximum rule over an entry's whole effect row, and the reporting rule for a door no schema models. Forge still gates on `entry.concurrency` alone in `fn plan_from_bytes`, so a `sequential` declaration over a `fx platform(atomic)` door publishes an end-to-end L3 artifact with zero residual machine assumptions. No atom maps to sequential, so the shipped `platform(clock)` fixtures `frozen_primitive_registry.json` and `separate_primitive_registry.json` are a second instance of the same laundering and the v1/v2 acceptance evidence needs a synthetic door whose effect row carries no platform atom, of the shape `fn foreign_identity` in `conformance/verified-build/boundary.th`. Implement the gate and turn the two pinned divergence tests green.<br>blockers: test:forge/tests/divergence_registry_v4_matrix.rs |
