...
The prior increments (REQ-1/3/4/5/6/7/8) stay SHIPPED. REQ-9/10/11 are GROUNDED (the verus
ports + the non-triviality mutations were RUN with the installed `verus --no-cheating`,
results in Grounding C/D/E) but **NOT-STARTED in-tree** — no `verus_core` extension, no
`lower_fn`/`aggregate`/`meets_floor` anchoring, no equivalence test has landed. Epic **#60**
owns the porting (no separate blocker — #60 is the tracker).

| REQ | Status | Evidence |
|---|---|---|
| REQ-9 (boundary HONESTY gate — Target C) | NOT-STARTED | epic #60. GROUNDED: the verus core `should_emit_external_body` (`ensures r == has_boundary \|\| has_slag` + the soundness corollary `(!has_boundary && !has_slag) ==> !r` + the global `regular_fn_never_external_body` proof) verifies `7 verified, 0 errors`; the broken-C mutant (exec body → `true`) reports `5 verified, 2 errors` (Grounding C). Gap: the production `lower_fn` gate (`f.boundary.is_some() \|\| f.slag.is_some()` in `thermite-lower/src/lower.rs`) is NOT yet anchored to the proved predicate, and no 4-combination equivalence test has landed. |
| REQ-10 (project LEVEL AGGREGATION — Target D) | NOT-STARTED | epic #60. GROUNDED: the verus core `aggregate_level` (the fold-min over `Seq<Level>`) + the two soundness lemmas `aggregate_le_all` (≤ every fn) and `aggregate_is_attained` (== the min) verify `7 verified, 0 errors`; the broken-D mutant (`min2` → MAX, an over-claim) reports `5 verified, 2 errors` (Grounding D). Gap: `AssuranceManifest::aggregate` (`forge/src/manifest.rs`) is NOT yet anchored to the proved fold-min by an exhaustive `Level`-list `verus_anchor` test (Option B, forge binary-only). The scope aggregation factors cleanly as an optional companion (OQ-D). |
| REQ-11 (mutation FLOOR gate — Target E, #48) | NOT-STARTED | epic #60. GROUNDED: the verus core `meets_floor_60` (INTEGER cross-multiply `scored > 0 && killed*100 >= scored*60`, `u128` exec widening, NO float) + the `scored == 0 ==> !r` #48 `ensures` + the global `zero_scored_never_passes` proof verify `7 verified, 0 errors`; the broken-E mutant (drop the `scored > 0` guard) reports `6 verified, 1 errors` (Grounding E). Gap: the production f64 `MutationScore::meets_floor` (`forge/src/mutation.rs`) is NOT yet anchored to the proved integer spec by the `0..=20 × 0..=20` f64↔integer `verus_anchor` grid (OQ-E — the f64-boundary equivalence must be RUN, not assumed). The existing #48 test `empty_score_is_below_floor` (0/0 gated) still passes. |
