...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (cache-key composition) | NOT-STARTED | open prereq blocker #8. No `forge/src/cache.rs` exists; the four-input sha256 `key` function is unwritten (`sha2` dep not yet added). |
| REQ-2 (soundness-completeness invariant) | NOT-STARTED | open prereq blocker #8. The hit-equals-fresh-verify guarantee depends on REQ-1's key, which does not exist. |
| REQ-3 (lookup-then-store flow, per item) | NOT-STARTED | open prereq blocker #8. `check::check_file` calls `run_verus` unconditionally on the L3 path; no cache lookup/store seam is wired. |
| REQ-4 (locality — per-item) | NOT-STARTED | open prereq blocker #8. `check::item_subprogram` already isolates per item (§5.3), but no key is derived from it. |
| REQ-5 (version-keyed invalidation) | NOT-STARTED | open prereq blocker #8. No verus-version capture (`verus --version`) or thermite-version (`CARGO_PKG_VERSION`) feeds any key. |
| REQ-6 (cache location + format) | NOT-STARTED | open prereq blocker #8. No `target/thermite-proof-cache/` store, no JSON-per-key load/store, no `.gitignore` confirmation. |
| REQ-7 (additive `cached: bool` field) | NOT-STARTED | open prereq blocker #8. `manifest::Certificate` has no `cached` field (only the #6 `slag_meta`/`reject` additive fields ship). |
| REQ-8 (bit-reproducible cert) | NOT-STARTED | open prereq blocker #8. The pipeline is deterministic per `.design/forge/check.md` REQ-7, but the cache-hit-equals-fresh-verify reproducibility property is unimplemented. |
