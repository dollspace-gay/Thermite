...
audited-content-sha256: 71c29565ee98397b13fa89dca183a2d7829d28d029b6ca7983a017688bb7bc79 (re-pinned 2026-08-07 after the quantified declared-index relation family landed the `#[logical]` attribute, its admission and relation gate, and the per-view `forall` emission)
...
> 11-ergonomics); the `struct`/`enum`/bare `#[sealed]`/explicit
> `#[sealed("factory")]`/`#[opaque]` item parses
> (`parse_struct`/`parse_enum`, 01-adts/06-provenance/
> `.design/build/opaque-library-state.md`); and the `?N` hole statement arm
