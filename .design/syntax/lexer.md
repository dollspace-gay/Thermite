...
| REQ-2 (keywords closed set) | SHIPPED | `keyword_kind` in `lexer.rs` maps the reserved words; effect/slag names fall through to `Ident`. The `break`/`continue` additions are REQ-10 (#93). |
...
| REQ-10 (`break`/`continue` keyword tokens, #93) | NOT-STARTED | open prereq blocker #93. `keyword_kind` in `lexer.rs` currently has NO `"break"`/`"continue"` arms, so both lex as `TokKind::Ident` (verified by reading `keyword_kind`: arms run `fn`…`is`/`true`/`false`, `_ => return None`). #93 must add `"break" => TokKind::Break` / `"continue" => TokKind::Continue` (two new `TokKind` variants) and the parser must recognize them (`parser.md` REQ-10). |
