...
- **REQ-7 (the Lean engine — discharge modes + termination)** — (i) AUTO: a tactic battery
  ...
  the certificate says PARTIAL-CORRECTNESS-only (tied to `while_rule`'s `h_run` premise),
...
- **AC-9 (the exec-body bridge design is DECISION-COMPLETE and spine-grounded — §4.1)** — every
  ...
  radius across the mutual denotation family; the optres position is OUT (no `ExecVal`
...
**Behavioral delta, stated — undelivered and not-yet-deliverable.** The seam is shipped; the
...
inspection tier. **NOT-STARTED — increment (ii)/(iv).**
...
COVERAGE, stated (the iv-b exportable-wider-than-dischargeable precedent): the LINEAR
...
the grammar stays `UntestedAgainstLean`. The accounting delta: `untested` SHRINKS from
...
the item's `dec` measure (the termination obligation class) — OR the certificate records
...
`Option<usize>` (IN — int payload). Recognizer EXP note, stated: unlike (v)'s
...
mutant stays `UntestedAgainstLean`. The floor gate begins to genuinely gate guard-return
