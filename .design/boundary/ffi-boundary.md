...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (surface form) | NOT-STARTED | open prereq blocker #16. Greenfield: a bodyless `fn … ;` fails to parse today — `thermite_syntax::parse` returns `Unexpected { expected: "{", found: ";" }` (`parse_fn` calls `parse_block` in `parser.rs`); no boundary form exists in `.design/syntax/surface-grammar.md` or `ast.rs`. |
| REQ-2 (AST shape) | NOT-STARTED | open prereq blocker #16. `FnItem` in `ast.rs` has `body: Block` (non-optional) and `slag: Option<SlagAttr>` but NO `boundary` field and NO `BoundaryAttr` type — a foreign-bodyless fn is unrepresentable. |
| REQ-3 (parser extension) | NOT-STARTED | open prereq blocker #16. `parse_slag` in `parser.rs` hardcodes `name == "slag"` — `#[boundary("…")]` fails to parse (`Unexpected { expected: "slag", found: identifier "boundary" }`, verified); `parse_fn` has no `Semi`-body path. |
| REQ-4 (L1 wrapper lowering) | NOT-STARTED | open prereq blocker #16. `l1.rs`'s `lower_fn_l1` always lowers a `f.body` block (`lower_fn_body_l1`); there is no foreign-call / bodyless arm and no `BoundaryAttr` to read a target from. |
| REQ-5 (forge cert L1 + boundary flag) | NOT-STARTED | open prereq blocker #16. `Certificate` in `manifest.rs` has `slag: bool` + `slag_meta` and `Certificate::slag_l1`, but NO `boundary` field, no `boundary_target`, and no `Certificate::boundary_l1`; `check.rs`'s `gate_fn` has a `slag.is_some()` branch but no `boundary` branch. |
| REQ-6 (#15 TCB hook) | NOT-STARTED | open prereq blocker #16. The audit surface enumerates `slag: true` (`slag.md`) but there is no `boundary` flag for #15's `slag ∪ boundary ∪ toolchain` enumeration to read. |
| REQ-7 (composition independence) | NOT-STARTED | open prereq blocker #16. No boundary fn can be declared, lowered, or certified, so no `caller`-through-contract path exists to exercise §9 composition. |
