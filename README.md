...
- ✅ **Acceptance programs** (`examples/`) — the compose-any-program proof: all four certify **L3** *and* `forge build`+run. A **verified interactive text editor** (`examples/editor/` — `render_frame`/`decode`/edit-core all L3, only the raw `read`/`write`/`ioctl` syscalls trusted; arrow-nav mid-text splice, `bash examples/editor/run.sh`), a `u64`→decimal **formatter**, a **calculator** (`parse_u64` + arithmetic), and a line/CSV **parser** (`split` → `Vec<String>`).
- 🔭 Deferred (tracked): direct MIR-level lowering + the incremental goal-state REPL (crosslink #21); ergonomic sugar (mutual recursion, tuple destructuring, `Map`, `for`-loops / match-guards / `if let`); the sandbox `ioctl` entry for a *confined* editor (#106); §7 equivalent-mutant exclusion (#101); and the basis v1.1 layer (dataflow taint-propagation, `Vec` element-invariants, scheme fusion).
