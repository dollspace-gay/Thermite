...
- 🔭 Deferred (tracked): the `forge body-tv` check phase wiring (#162); direct MIR-level lowering + the incremental goal-state REPL (crosslink #21); the basis v1.1 layer (dataflow taint-propagation, `Vec` element-invariants, scheme fusion); and the proof-spine residuals, each named in its doc — full Z3 demotion (upstream-gated: Lean-SMT's QF_BV `sorry`, no Verus certificates), the Lean→Rust extraction bridge, user-ADT `match`/`is` in the proven fragment.
