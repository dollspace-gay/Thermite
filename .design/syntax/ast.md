...
| REQ-6 — CHAR/HEX/BIN reuse `IntLit` (#91/#92) | NOT-STARTED | blocker #92. The `Expr::IntLit` STRUCT-SHAPE is shipped, but no lexer/parser path produces `'A'`/`0x1b`/`0b101` into it yet (`lexer.md` REQ-3/REQ-9 NOT-STARTED). The contract pinned: char/hex/binary REUSE `Expr::IntLit` (NO new variant); builder wires the lexer→`IntLit` path. GROUNDED L3. |
...
| REQ-10 (binary + unary operator set, #92) | NOT-STARTED | blocker #92. `enum BinOp` in `ast.rs` today has ONLY `Add/Sub/Mul/Div/Eq/Ne/Lt/Le/Gt/Ge/And/Or` — NO `Rem`/`Shl`/`Shr`/`BitAnd`/`BitOr`/`BitXor`; there is NO `UnaryOp` enum and NO `Expr::Unary` node (`!` standalone is unhandled; `Bang` only feeds `!=`). Builder adds the 6 binops + `UnaryOp::Not` + `Expr::Unary` (→ match-arm ripple, Architecture). GROUNDED L3 for all forms. |
| REQ-11 (partial-operator obligations, #92) | NOT-STARTED | blocker #92. No `/`/`%`/`<<`/`>>` obligation handling exists because `%`/`<<`/`>>` don't parse yet. `/` (`BinOp::Div`) already lowers to bare Verus `/` (`binop` in `lower.rs`), so its divide-by-zero obligation ALREADY bites (GROUNDED: `/` without `req b != 0` is L0). The NEW partials (`%`/shifts) inherit the same Verus-native obligation. Builder must keep emitting bare operators (no `external`/`assume`). |
