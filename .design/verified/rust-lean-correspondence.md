...
### 2D. Exact fixed storage + executable calls — `ExecRefCtx` ↔ `Exec.KernelAggregate`

| Construct | Rust arm | Lean arm | Bridge | Pinned by |
|---|---|---|---|---|
| `FixedArray8.get(i)` | `encode_method_call` admits `.get(i)` only when `expr_type(receiver)` resolves through an exact `TFixedArray8*` declaration; emits `receiver.spec_get(i)` | `fixedArrayGetRef` interprets the reference projection; `fixedArrayGetDenote` independently dispatches the source operation over the eight named slots | both return exactly slot `i`; an out-of-range index has no value | `fixed_array_get_ref_sound`; live direct-`FixedArray8` Verus frame regression |
| framed user call | `encode_call` requires `ctx.calls[name]`, checks exact arity, and narrows every argument at its declared type; no declaration is `Unsupported` | `encodeExactCall` requires `ExactCallFrame name = some decl` and exact arity; `denoteExactCallToken` re-applies the declaration and its precondition | the emitted call token has only the value meaning supplied by its receipt-bound declaration | `exact_call_ref_sound`; `unframed_call_is_rejected`; negative `permissive_unframed_call_breaks_refinement` |
| prior-local callee precondition | `ExecObligationFrame.local_assumptions` is emitted in the obligation `requires`, preserving lexical facts needed by a staged call | `ExactCallDecl.pre`; a call denotes only when the exact precondition holds | an exact signature alone cannot invent the caller-state fact required by the callee | `prior_local_assumption_enables_exact_call`; `missing_prior_local_assumption_blocks_call`; live Forge under-framed staged-call tooth |

---

## Table 3 — `thermite-tv/src/exec_stmt_encode.rs` ↔ `lean/Thermite/Exec/{Stmt,KernelAggregate}.lean`
...
| owned named aggregate + `FixedArray8.set` | `aggregate_mutation_reference` recognizes exactly `let mut updated: S = base; updated.arr.set(i,v); updated.field = …; updated`, resolves reads against the prior aggregate state, and reconstructs all eight array slots plus every declared result field | `FixedArray8Val` is the exact eight-slot generated representation; `fixedArraySetDenote` source-dispatches one slot while `fixedArraySetRef` independently reconstructs all slots; `KernelAggregateVal` composes the array update while retaining other fields | the generated reference changes exactly the selected slot and explicit whole-field updates; no collateral cell is hidden by aggregate equality | `fixed_array_set_ref_sound`; `aggregate_set_ref_sound`; negatives `wrong_fixed_array_index_breaks_refinement` / `wrong_fixed_array_value_breaks_refinement`; live faithful/wrong-index/wrong-value Verus teeth |
| other non-bare target | a range/projection/unframed indexed target, a different aggregate shape, or a read after an in-place fixed-array update → `Err(Unsupported)` | absent from the admitted companion model | remains OUT | n/a (faithful absence) |
...
| aggregate tail | `exec_ref_ensures_value` projects every declared result field; a `TFixedArray8*` field expands to eight `spec_get` equalities | `KernelAggregateVal`/`FixedArray8Val` retain every named scalar and slot; `aggregate_set_ref_sound` is structural equality of the complete value | per-field/per-slot Verus packaging denotes complete aggregate equality | `aggregate_set_ref_sound` plus the three live fixed-array Verus teeth |
