...
fn lower_expr(expr: &Expr, ctx: Ctx, depth: usize, span: Span) -> Result<String, LowerError> {
    ...
    match expr {
        ...
        Expr::Call { callee, args } => {
            ...
            let string_as_byteview = callee_takes_string_byteview(callee);
...
/// String-scanning twin `spec_line_start`/`count_x`). Keyed on the callee NAME
/// (these names are reserved by the generated defs — no user collision in v0.1).
fn callee_takes_string_byteview(callee: &Expr) -> bool {
