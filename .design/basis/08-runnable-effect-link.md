...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (the `os::<name>` wrapper stdlib — real `std` syscall bodies) | NOT-STARTED | issue #81, Stage 8 (GREENFIELD). No `thermite-stdlib` crate exists in the tree (`ls thermite-stdlib` → absent; not a workspace member in `Cargo.toml`); no `os::now`/`os::read_byte`/`os::write` wrapper is defined. The wrapper SHAPE is GROUNDED runnable (`os::now` → `std::time::SystemTime` compiles + runs a real `clock_gettime`; `os::read_byte` → `std::io::stdin().read` compiles + runs, byte 'A' → 130, EOF → 0), but no wrapper source ships. |
| REQ-2 (`forge build` LINKS via emit-`mod os` keyed off boundary targets) | NOT-STARTED | issue #81. GROUNDED gap: `forge build time_demo.th --entry elapsed_ok` → `error[E0433]: cannot find module or crate \`os\`` at `let result = os::now();`. `forge/src/build.rs` `emit_source`/`synthesize_entry_main` emit `lower_l1` + the #57 prelude but NO `mod os` (Read confirms: no `os` emission in `build.rs`); `lower_boundary_fn_l1` (`thermite-lower/src/l1.rs`) emits the `os::<name>(args)` crossing the link must resolve. The emit-`mod os` fix is GROUNDED to close the `E0433` (the manual `mod os` → rustc exit 0). |
| REQ-3 (a verified program COMPILES + RUNS + does real I/O) | NOT-STARTED | issue #81. GROUNDED that the linked path RUNS (`elapsed_ok() = 1780779938`, a live timestamp; `doubled() = 130`/`0` over stdin), but the link is not in `forge build` yet (REQ-2), so `forge build --entry` of a real caller still `E0433`s. The SHIPPED `--entry` runnable form + baked-in L1 checks (`build.md` REQ-3/REQ-4, `synthesize_entry_main` in `build.rs`) carry the run scaffolding once the link lands. |
| REQ-4 (the linked wrapper is #57-seccomp-CONFINED) | NOT-STARTED | issue #81. The SHIPPED #57 `sandbox::emit_sandbox_prelude` over `transitive_fx`/`syscall_allowlist` (`forge/src/sandbox.rs`) installs the `fx`-derived filter FIRST in `synthesize_entry_main`'s `main` TODAY; GROUNDED that it confines the linked wrapper (the `time`-allowlisted `clock_gettime` runs, exit 0; an out-of-`fx` `openat` under the `time` filter → `SIGSYS`, exit 159). But no live foreign-body program exercises it (the link, REQ-2, is absent) — Stage 3 demonstrated confinement only via the `--sandbox-self-test` probe, not a live `os::` call. |
| REQ-5 (verification UNCHANGED — link is build-only) | NOT-STARTED | issue #81. GROUNDED on the SHIPPED tree that `forge check time_demo.th --mutation-floor 0` certifies `now` L1 + `boundary_target os::now` and `elapsed_ok` L3 + `to-the-boundary (via now)` independent of any link (the link does not exist and check still certifies). The claim "the link adds no verification path" is structurally true (the link lives in `build.rs` codegen + rustc; `forge check` never emits `mod os` or invokes rustc), but no Stage-8 oracle PINS the before/after invariance yet. |
| REQ-6 (the wrappers are the TRUSTED-by-fiat TCB — enumerated + confined) | NOT-STARTED | issue #81. GROUNDED that `forge audit time_demo.th` enumerates `boundary: now -> os::now (req=… ens=… fx=[time])` in the TCB (the SHIPPED #15 `AuditManifest.tcb`), and that #57 confines the wrapper (REQ-4). But no Stage-8 corpus links a wrapper and PINS that the linked, runnable wrapper is exactly the enumerated, confined boundary — there is no wrapper to link yet. |
