...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (loop surface + v1 frozen loop subset) | NOT-STARTED | open prereq blocker #163. The loop AST is SHIPPED (`struct LoopNode`/`enum LoopKind`/`Stmt::Loop` in `thermite-syntax/src/ast.rs`; lowered by `lower_loop in lower.rs`); the v1 SUBSET (single `while` + declared inv/dec + straight-line scalar body; `loop`-kind / break / continue / mid-body-return / nested / non-scalar OUT) is PINNED here but NOT yet pinned in `thermite-tv` code (a loop is currently a blanket `RefEncodeError::Unsupported` in `body_ref_state` — the honest 2.2.1 skip, not the v1 loop arm). |
| REQ-2 (Rust loop TV — the three per-run obligations) | NOT-STARTED | open prereq blocker #163. No `loop_ref_obligations`/`LoopObligationFrame` exists; `grep` for `loop_ref` in `thermite-tv/src/` is empty. The single-iteration body step it REUSES is SHIPPED: `body_ref_state in exec_stmt_encode.rs` (the straight-line block transformer). Designed above; unbuilt. |
| REQ-3 (Lean iteration semantics + WHILE-RULE) | NOT-STARTED | open prereq blocker #163. No `lean/Thermite/Exec/Loop.lean` exists (`ls lean/Thermite/Exec/` = `Stmt.lean`, `StmtDivergence.lean` only). The single-iteration step it iterates is SHIPPED + PROVEN: `blockThread`/`body_ref_sound in Exec/Stmt.lean` (#172). The `h_tv` capstone pattern it extends is SHIPPED: `tv_meta_body`/`FnTvWitness in Faithfulness.lean` (#174/#183). Designed above; unbuilt. |
| REQ-4 (trust accounting — partial correctness, termination residual) | NOT-STARTED | open prereq blocker #163. The DECISION (partial correctness for v1; termination stays the per-run Verus `decreases` discharge, `lower_loop`'s SHIPPED `decreases <dec>` emission) is RECORDED above; the WHILE-RULE that carries the `loop EXITS` hypothesis is REQ-3 (unbuilt). |
| REQ-5 (increment plan + forge `body_tv` loop wiring) | NOT-STARTED | open prereq blocker #163; the forge seam is blocker #162 (the `body_tv` phase scaffold — itself NOT-STARTED, no `forge/src/body_tv.rs`). The build order (Rust obligations → Lean WHILE-RULE → forge wiring) + the per-increment manifests + ACs are pinned above; unbuilt. |
