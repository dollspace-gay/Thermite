...
(`thermite-spec/src/validator.rs` — `len`/`get`/`last`/`contains`/… only). Every
REQ here is **NOT-STARTED** behind a filed blocker. The full Verus path was
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (`Map<K,V>` two-arg `Type` node + grammar) | NOT-STARTED | blocker #123. `enum Type` (`thermite-syntax/src/ast.rs`) has no `Map` arm (carries `Prim`/`Unit`/`Ref`/`Slice`/`Generic`/`Named`/`Box`/`Vec`/`String`/`Option`/`Result`/`Tuple` only); `parse_type` has no `"Map"` contextual-ident arm. `Map<u64, u64>` does not parse — the single-arg `Generic` dies at the comma (the C7 finding). The two-arg `Type::Result(Box, Box)` precedent (SHIPPED, `.design/basis/09-option-result.md` REQ-2) is the exact shape to mirror. |
| REQ-2 (`Map` ops are `Expr::MethodCall` — no new node) | NOT-STARTED | blocker #123. `Expr::MethodCall` already lowers (the SHIPPED `Vec`/`String` accessors), so no new `Expr` node is needed — but `insert`/`get`/`contains_key`/`len` over a `Map` cannot be written until REQ-1's `Type::Map` parses. Reaches the surface only with REQ-1. |
| REQ-3 (`contains_key`/`insert` cage admission; capacity/op contracts in §4.2) | NOT-STARTED | blocker #123. `contains_key` is NOT in `BUILTIN_METHODS` (`thermite-spec/src/validator.rs` carries `len`/`get`/`last`/`contains`/`byte_at`/`concat`/`slice`/`push_byte`/`to_string`/`starts_with`/`ends_with`/`find`/`split`/`trim` — no `contains_key`). The cage walk (`walk_expr_inner`) and the C7 spec-`match` accept path are SHIPPED and unchanged-feasible (design-confirmed), but `contains_key` must be added before a `Map` round-trip contract validates. |
| REQ-4 (`Map` → Vec-of-pairs wrapper + spec view; ops; `fx alloc`) | NOT-STARTED | blocker #123. `lower.rs` has no `emit_map_wrappers` / `tmap_name` / `Type::Map` `lower_type` arm. The GROUNDED `TMapU64U64` form (Verification — `8 verified, 0 errors`, the insert-then-get round-trip + absent→None) is design-confirmed feasible and is the exact wrap-a-`Vec` form `emit_vec_wrappers` mirrors; not yet emitted. The `fx alloc` rule (the `Vec`-`push` / `Effect::Alloc`) is SHIPPED and REUSED. |
| REQ-5 (`Type::Map` exhaustive-match + skill ripple) | NOT-STARTED | blocker #123. The new two-arg `Type::Map` variant breaks every exhaustive `match Type` (the SHIPPED `Type::Result`/`Type::Tuple` ripple set: `parser.rs`, `ast.rs`, `lower.rs`, `l1.rs`, `l2.rs`, `forge/src/{check,mutation,review}.rs`, `thermite-skill/src/generate.rs` — grep-confirmed). No `Type::Map` arm exists at any site yet; the skill 6,000-token budget gate must still pass with the new fragment. |
| REQ-6 (`LowerError`/`SpecError` extension, no panics) | NOT-STARTED | blocker #123. Reuses the EXISTING `LowerError::Unsupported` (an unsupported `(K, V)` monomorphization) — no new variant expected — but the `Map` lowering path that would invoke it does not exist (REQ-4). No `unwrap`/`expect`/`panic!` to be added (R-CODE-2 / R-APG-1). |
