...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (exec-expr reference encoder) | NOT-STARTED | open prereq blocker #152. `thermite-tv/src/exec_encode.rs` does not exist; the existing `thermite-tv/src/ref_encode.rs` (`ref_contract_pred`) is CONTRACT-position only (slice→`@`, method→`spec_*`, cast→`nat`/`int`) — it encodes spec/nat semantics, the wrong semantics for an exec VALUE. The exec reference (bounded `u64`/`usize`, no `@`/`nat`) is a new sibling encoder. |
| REQ-2 (exec-fn-wrapped equivalence obligation + discharge) | NOT-STARTED | open prereq blocker #153. `thermite_tv::obligation::equivalence_obligation` (`thermite-tv/src/obligation.rs`) emits the CONTRACT `proof fn { assert((P_prod) <==> (P_ref)); }` form — a predicate equivalence, NOT an exec-value `ensures`. The new `exec_equivalence_obligation` emits the `fn tv_exec_wrap(..) ensures result == <ref> { <p_prod> }` exec-fn form (GROUNDED VERIFIED faithful / counterexample on the #122/#146/overflow cases under real verus, see Verification). |
| REQ-3 (off-corpus generator — exec exprs) | NOT-STARTED | open prereq blocker #154. `thermite_tv::gen::generate_clauses` (`thermite-tv/src/gen.rs`) generates CONTRACT-position `bool` `Expr`s (the cast-`<` it emits is for the contract obligation). The new `gen_exec_exprs` generates EXEC-position value exprs (u64/usize arithmetic, narrowing/widening casts, indexing of `&[u32]`) for the exec-fn obligation. |
| REQ-4 (the teeth — R-CHAR-3) | NOT-STARTED | open prereq blocker #155. `thermite-tv/tests/exec_teeth.rs` does not exist; the existing `thermite-tv/tests/teeth.rs` discharges CONTRACT obligations (F1–F4). The exec teeth (#122 paren-drop → E0308/postcondition, #146 cast-`<` → `expected ','`, overflow → postcondition) are GROUNDED here under real verus but not yet a replayable test. |
| REQ-5 (forge plug-in point) | NOT-STARTED | open prereq blocker #156. `forge/src/exec_tv.rs` does not exist; `forge/src/contract_tv.rs` (`tv_file`/`run_generated`) runs the CONTRACT obligation over `req`/`ens`/`inv`/`dec` clauses via `thermite_lower::lower_contract_expr` (SPEC context). The exec phase lowers body exec exprs in EXEC context (the per-expr exec-lowering entry — reuse `lower_contract_expr` in an exec `Ctx` or add a `lower_exec_expr`, the #152-noted prerequisite) and discharges the exec-fn obligation through `forge::check::run_verus`. |
