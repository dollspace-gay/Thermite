...
| REQ-3 — RAW (verbatim source slice on the token, #37) | SHIPPED | `lex_int` in `lexer.rs` captures the verbatim digit+`_` run (`source[i..last_digit]`) as `raw`; `TokKind::Int { value: u128, raw: String }` so `1_000_000` lexes to value `1000000` AND raw `"1_000_000"`, and a trailing-`_` (`1_`) to value `1`/raw `"1"` (AC-2b). Consumer: `parse_primary`/pattern-literal in `parser.rs`; test `int_literal_preserves_raw` in `tests/conformance.rs`. |
